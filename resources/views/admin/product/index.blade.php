        ...
        <div class="card-body">
            <form action="/admin/san-pham" method="get">
                ...
                <div class="form-search row align-items-end">
                    ...
                    <div class="col-12 col-md-3">
                        <label for="choices-sex-input" class="form-label" style="clear: both; display: inline-block; white-space: nowrap;">Phân loại sp</label>
                        <select class="form-select" name="root_id" id="choices-sex-input">
                            {!! $categoryParentHtml !!}
                        </select>
                    </div>
            ...
            <div class="table-responsive table-card mt-3 mb-1">
                <table class="table align-middle table-nowrap" id="customerTable">
                    ...
                    <tr>
                        ...
                        <th  data-sort="address">Phân loại sp</th>
                        <th  data-sort="address">Danh mục sp</th>
                    ...
                    @foreach($data as $index => $item)
                        <tr>
                            ...
                            <td class="customer_name">{{ $item->categories()?->first()->parent?->title }}</td>
                            <td class="customer_name">{{$item->categories()->first()->title}}</td>
                            <td class="customer_name">{{ $item->createdBy?->email }}</td>
                            <td class="customer_name">{{ $item->ordering }}</td>
                            <td>
                                <div class="d-flex gap-2">
                                    <div class="edit">
                                        <button onclick="window.location='/admin/{{$item->id}}/san-pham'" class="btn btn-sm btn-success edit-item-btn" >Cập nhật</button>
                                    </div>
                                    <div class="remove">
                                        <button
                                            value="{{ $item->id }}"
