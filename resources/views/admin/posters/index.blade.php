...
@section('content')
    <div class="card rounded-3">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h6 class="mb-0">Danh sách poster</h6>
            <a href="/admin/them-moi/poster" class="btn btn-dark waves-effect waves-light w-sm pt-2 pb-2">Thêm mới</a>
        </div>
    </div>
    ...
    <div class="card mt-2">
        <div class="card-body">
            <div id="customerList">
                <form action="/admin/posters" method="get">
                    <div class="form-search row align-items-center row-gap-3">
                        <div class="col-12 col-md-4 d-flex align-items-center gap-3">
                            ...
                            @foreach(request()->query() as $key => $value)
                                ...
                                @endif
                            @endforeach
                            <input class="form-control" value="@if(isset($input['name'])){{$input['name']}}@endif" placeholder="Nhập tên poster" name="name" id="search-name"/>
                        </div>
                        ...
                        <div class="col-3 col-md-2 offset-md-4 d-flex align-items-center gap-3">
                ...
                <div class="table-responsive table-card mt-3 mb-1">
                    <table class="table align-middle table-nowrap" id="customerTable">
                        ...
                        <tr>
                            ...
                            <th  data-sort="customer_name">Tiêu phụ đề</th>
                            <th  data-sort="customer_name">Mô tả ngắn</th>
                        ...
                        @foreach($data as $index => $item)
                            <tr>
                                ...
                                <td class="customer_name">{{ $item->subtitle }}</td>
                                <td class="customer_name">{{ $item->description }}</td>
                                <td class="customer_name">{{ $item->ordering }}</td>
                                <td>
                                    <div class="d-flex gap-2">
                                        <div class="edit">
                                            <button onclick="window.location='/admin/{{$item->id}}/poster'" class="btn btn-sm btn-success edit-item-btn" >Cập nhật</button>
                                        </div>
                                        <div class="remove">
                                            <button
                                                value="{{ $item->id }}"
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#delete-record').click(function () {
                ...
                form.attr('action', '/admin/'+ item_id+'/poster');
                form.submit();
            });
        })
    </script>
