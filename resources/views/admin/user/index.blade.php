...
@section('content')
    <div class="card rounded-3">
        <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <a href="/admin/them-moi/user" class="btn btn-dark waves-effect waves-light w-sm pt-2 pb-2">Thêm mới</a>
        </div>
    </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex gap-2">
                                    <div class="edit">
                                        <button class="btn btn-sm btn-success edit-item-btn" data-value="{{$item->id}}" >Khôi phục mật khẩu</button>
                                    </div>
                                    <div class="remove">
                                        <button
                                            value="{{ $item->id }}"
    ...
    <!-- Modal delete -->
    <div class="modal fade zoomIn" id="resetRecordModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"
                ...
                <div class="modal-body">
                    <form method="POST" id="form-reset-password">
                        ...
                        <div class="mt-2 text-center">
                            ...
                            <div class="mt-4 pt-2 fs-15 mx-4 mx-sm-4">
                                <h4>Bạn đã chắc chắn ?</h4>
                                <p class="text-muted mx-4 mb-0">Bạn có chắc chắn muốn khôi phục về mật khẩu mặc định ?</p>
                            </div>
                        </div>
                        <div class="d-flex gap-2 justify-content-center mt-4 mb-2">
                            <button type="button" class="btn w-sm btn-light" data-bs-dismiss="modal">Đóng</button>
                            <button type="button" class="btn w-sm btn-danger " id="reset-record">Đồng ý!</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#delete-record').click(function () {
                ...
                form.attr('action', '/admin/'+ item_id+'/user');
                form.submit();
            });
            ...
            $('.edit-item-btn').click(function (e) {
                ...
                if (userId) {
                    $('#user_id').val(userId);
                    $('#resetRecordModal').modal('show');
                }
            })
            $('#reset-record').click(function () {
                const item_id = $('#user_id').val();
                if (item_id) {
                    const form = $('#form-reset-password');
                    form.attr('action', '/admin/'+item_id+'/user/reset');
                    form.submit();
                }
            })
        })
