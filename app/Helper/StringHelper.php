...
class StringHelper
{
    public static function getSelectOption($options ,$selected = '', $firstOption="Vui lòng chọn", $firstSelected = true, $addFirstOption=true, $field = 'title'){
        $html = '';
        if ($addFirstOption) {
        ...
        foreach ($options as $option) {
            $html .= '<option value="' . $option->id . '"' . ((is_array($selected) ? in_array($option->id, $selected) : $selected == $option->id) ? 'selected' : '') . '>' . $option[$field] . '</option>';
        }
        return $html;
    }
